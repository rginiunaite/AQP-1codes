//
// Created by rasa on 15/10/2020.
//

/*
* Code for all the simulations with nice descriptions, this code is described as a pseudocode. Change the extent of the boundary, wide VEGF boundary!!!
*/
...
VectorXd proportions(double diff_conc, int n_seed) {
    ...
    double domain_length = 30; //this variable is for the actual domain length, since it will be increasingS
    ...
    double speed_l = 0.14;//0.14;// 0.05;//1;//0.05; // speed of a leader cell // AQP-1 downregulation 0.08., control - 0.09, AQP-1 upregulation 0.14
...
//    double constant = 29.12 ;
    ...
    double boundlen = 50.0; // if boundary is false, then this is the lenght of hard boundary, 20.0!!!!!
    double chemooutside = 0.0;
    /*
     * initialise a matrix that stores values of concentration of chemoattractant
     * */

    MatrixXf chemo = MatrixXf::Zero(length_x, 3*length_y);
    MatrixXf chemo_new = MatrixXf::Zero(length_x, 3*length_y);

    // initialise internalisation matrix
    MatrixXf intern = MatrixXf::Zero(length_x, length_y*3);

    for (int i = 0; i < length_x; i++) {
        for (int j = length_y; j < 2*length_y; j++) {
    ...
    MatrixXf chemo_3col(length_x * 3*length_y, 4), chemo_3col_ind(length_x * 3*length_y,
                                                                  2); // need for because that is how paraview accepts data, third dimension is just zeros

    // x, y coord, 1st and 2nd columns respectively
    int k = 0;
    // it has to be 3D for paraview
    while (k < length_x * 3*length_y) {
        for (int i = 0; i < length_x; i++) {
            for (int j = 0; j < 3*length_y; j++) {
    ...
    for (int i = 0; i < length_x * 3*length_y; i++) {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        get<position>(particles[i]) = vdouble2(cell_radius, length_y+(i + 1) * double(length_y-1 - cell_radius) / double(N) -
    ...
    particles.init_neighbour_search(vdouble2(-50, -50), 5 * vdouble2(length_x, 3*length_y), vbool2(false, false));
    ...
    for (int t = 0; t < N_steps; t++) {
        ...
        if (t % insertion_freq == 0) {
            ...
            get<position>(f) = vdouble2(cell_radius, uniform(gen)+length_y); // x=2, uniformly in y
            free_position = true;
            /*
        ...
        for (int ka =0; ka< number_time;ka++) {
            ...
            domain_length =
                    ((L_inf * exp(a * (dt / 60.0 - t_s))) / (L_inf / L_0 + exp(a * (dt / 60.0 - t_s)) - 1)) + constant;

            domain_len_der = (
                    (a * L_inf / 60 * exp(a * (dt / 60.0 - t_s))) / (L_inf / L_0 + exp(a * (dt / 60.0 - t_s)) - 1) -
                    (a * L_inf / 60 * exp(2 * a * (dt / 60.0 - t_s))) /
                    ((L_inf / L_0 + exp(a * (dt / 60.0 - t_s)) - 1) * (L_inf / L_0 + exp(a * (dt / 60.0 - t_s)) - 1)));
...
//                              (a * L_inf * exp(2 * a * (dt - t_s))) / ((L_inf / L_0 + exp(a * (dt - t_s)) - 1)*(L_inf / L_0 + exp(a * (dt - t_s)) - 1)));
            ...
            if (boundary == true) {
                ...
                for (int i = 1; i < length_x - 1; i++) {
                    for (int j = length_y+1; j < length_y * 2-1 ; j++) {


                        // logistic production rate

                        chemo_new(i, j) =
                                dt_init * (D * ((1 / ((domain_length / length_x) * (domain_length / length_x))) *
                                                (chemo(i + 1, j) - 2 * chemo(i, j) + chemo(i - 1, j)) /
                                                (dx * dx) +
                                                (chemo(i, j + 1) - 2 * chemo(i, j) + chemo(i, j - 1)) /
                                                (dy * dy)) -
                                           (chemo(i, j) * lam / (2 * M_PI * R * R)) * intern(i, j) +
                                           kai * (chemo(i, j)) -
                                           double(domain_len_der) / double(domain_length) * chemo(i, j)) +
                                chemo(i, j);
            ...
            if (boundary == false) {
                ...
                for (int i = 1; i < length_x - 1; i++) {
                    if ( i * domain_length/length_x < boundlen) {
                        for (int j = length_y+1; j < length_y * 2-1 ; j++) {
            ...
            if(boundary == true){
                for (int i = length_y; i < 2*length_y; i++) {
                ...
                for (int i = 0; i < length_x; i++) {
                    chemo_new(i, length_y) = chemo_new(i, length_y+1);
                    chemo_new(i, 2*length_y - 1) = chemo_new(i, 2*length_y - 2);
                }
                ...
                for (int i = 0; i < length_x; i++) {
                    if (i*domain_length/length_x < boundlen){
                        ...
                        chemo_new(i, 3*length_y - 1) = chemo_new(i, 3*length_y - 2);
                    }
        ...
        for (int i = 0; i < length_x * length_y*3; i++) {
        ...
        if (t % freq_growth == 0) {
            ...
            for (int i = 0; i < particles.size(); i++) {
              vdouble2 xposition = get<position>(particles)[i]*vdouble2((domain_length / old_length), 1);
              xposition[0] = (length_x / domain_length) * xposition[0];
              if (xposition[0]< length_x-cell_radius){
...
//            }
            ...
            if (get<type>(particles[particle_id(j)]) == 0) {
                ...
                if (get<persistence_extent>(particles[particle_id(j)]) == 1) {
                    ...
                    if (boundary == true){
                        if (free_position && x[0] > cell_radius && x_in < length_x - cell_radius && (x[1]) > cell_radius +length_y&&
                    ...
                    if (boundary == false){
                        if (x[0] < boundlen){
                            ...
                            if (free_position && x[0] > cell_radius && x_in < length_x - cell_radius && (x[1]) > cell_radius &&
                ...
                if (get<persistence_extent>(particles[particle_id(j)]) == 0) {
                    ...
                    for (int i = 0; i < filo_number; i++) {
                        ...
                        if (round((x_in + sin(random_angle[i]) * l_filo_x)) < 0 ||
                            ...
                            round(x[1] + cos(random_angle[i]) * l_filo_y) > length_y*3 - 1) {
                            new_chemo[i] = 0;
                        } else {
...
//                        // SHORT BOUNDARY
            ...
            if (get<type>(particles[particle_id(j)]) == 1) {
                ...
                if (get<chain>(particles[particle_id(j)]) > 0) {
                    ...
                    // update the position if the place they want to move to is free and not out of bounds
                    if (boundary == true) {
                        if (free_position && x_chain[0] > cell_radius && x_in_chain < length_x-cell_radius && (x_chain[1]) > cell_radius + length_y &&
                    ...
                    if (boundary == false) {
                        if (x_chain[0] < boundlen) {
                            ...
                            if (free_position && x_chain[0] > cell_radius && x_in_chain < length_x-cell_radius && (x[1]) > cell_radius &&
                                                 (x[1]) < length_y *3 - cell_radius) {
                                get<position>(particles)[particle_id(j)] +=
                                        increase_fol_speed * get<direction>(particles[particle_id(j)]);
                    ...
                    if (get<chain>(particles)[particle_id(j)] == 0) {
                        ...
                        */
                        ...
                        if (get<chain>(particles[particle_id(j)]) > 0) {
                            ...
                            if (boundary == true) {
                                if (free_position &&
                                    (x_in_chain) > cell_radius*(length_x/domain_length) &&
                            ...
                            if (boundary == false) {
                                if (x_in_chain < boundlen) {
                                    if (free_position &&
                                        (x_in_chain) >  cell_radius*(length_x/domain_length) &&
                                        (x_in_chain) < length_x - cell_radius  && (x_chain[1]) >  length_y + cell_radius &&
                                        (x_chain[1]) < 2*length_y - cell_radius) {
                                        //cout << "direction " << get<direction>(particles[particle_id(j)]) << endl;
                                        get<position>(particles)[particle_id(j)] +=
                                                increase_fol_speed * get<direction>(particles[particle_id(j)]);
                                    ...
                                    if (free_position &&
                                        (x_in_chain) >  cell_radius*(length_x/domain_length) &&
                                        (x_in_chain) < length_x - cell_radius  && (x_chain[1]) > cell_radius &&
...
//                        vdouble2 x_can = direc_follow;
                            ...
                            if (boundary == false) {
                                ...
                                if (x_can[0] < boundlen) {
                                    ...
                                    if (free_position && x_can[0] * (length_x / domain_length) >  cell_radius*(length_x/domain_length) && (x_can[0] * (length_x / domain_length)) < length_x-cell_radius
                              && (x_can[1]) > cell_radius + length_y &&
                                    ...
                                    if (free_position &&
                                        ((x_can[0] * (length_x / domain_length))) >  cell_radius*(length_x/domain_length) &&
                            ...
                            if (get<in_track>(particles[particle_id(j)]) == 0) {
                                ...
                                if (boundary == true) {
                                    ...
                                    if (free_position && (x_in) > cell_radius*(length_x/domain_length) &&
                                        ...
                                        //cout << "here after " << get<position>(particles)[particle_id(j)] <<  endl;
                                    }
                                }
                                if (boundary == false) {
                                    if (x[0] < boundlen) {
                                        ...
                                        if (free_position && (x_in) >  cell_radius*(length_x/domain_length) &&
                                            (x_in) < length_x - cell_radius && (x[1]) > cell_radius+length_y &&
                                            (x[1]) < 2*length_y - cell_radius) {
                                            //cout << "how frequently come in here " << endl;;
                                            get<position>(particles)[particle_id(j)] +=
                                        ...
                                        if (free_position && (x_in) >  cell_radius*(length_x/domain_length) &&
                                            (x_in) < length_x - cell_radius && (x[1]) > 0 &&
                    ...
                    if (get<chain>(particles[particle_id(j)]) == 0) {
                        ...
                        */
                        ...
                        if (get<chain>(particles[particle_id(j)]) > 0){
                            ...
                            if (boundary == true){
                                if (free_position &&
                                    x_in_chain > cell_radius*(length_x/domain_length) && x_in_chain < length_x-cell_radius && (x_chain[1]) > cell_radius +length_y&&
                                    (x_chain[1]) < length_y*2 -cell_radius) {
                                    //cout << "direction " << get<direction>(particles[particle_id(j)]) << endl;
                                    get<position>(particles)[particle_id(j)] +=
                                            increase_fol_speed * get<direction>(particles[particle_id(j)]);
...
//                        // short boundary
                            if (boundary == false){
                                if(x_chain[0]< boundlen){
                                    ...
                                    if (free_position &&
                                        x_in_chain >  cell_radius*(length_x/domain_length) && x_in_chain < length_x-cell_radius  && (x_chain[1]) > cell_radius &&
                                        (x_chain[1]) < length_y*3 - cell_radius ) {
                                        //cout << "direction " << get<direction>(particles[particle_id(j)]) << endl;
                                        get<position>(particles)[particle_id(j)] +=
                                                increase_fol_speed * get<direction>(particles[particle_id(j)]);
...
//                    }
                        ...
                        if (get<chain>(particles[particle_id(j)]) == 0) {
                            ...
                            //full boundary
                            if (boundary == true){
                                if (free_position &&x_in >  cell_radius*(length_x/domain_length) && x_in < length_x - cell_radius  && (x[1]) > cell_radius+length_y &&
                                    (x[1]) < length_y*2 - cell_radius) {
                                    get<position>(particles)[particle_id(j)] += speed_f * vdouble2(sin(random_angle),
                                                                                                   cos(random_angle)); // update if nothing is in the next position
...
//                        // short boundary
                            if (boundary == false) {
                                if (x[0] < boundlen) {
                                    ...
                                    if (free_position && x_in >  cell_radius*(length_x/domain_length) && x_in < length_x-cell_radius && (x[1]) > cell_radius &&
                                                         (x[1]) < length_y *3 - cell_radius) {
                                        get<position>(particles)[particle_id(j)] += speed_f * vdouble2(sin(random_angle),
                                                                                                       cos(random_angle)); // update if nothing is in the next position
                                        get<direction>(particles)[particle_id(j)] = speed_f * vdouble2(sin(random_angle),
...
//
//#ifdef HAVE_VTK
//            vtkWriteGrid("WideBoundary500Boundcells", t, particles.get_grid(true));
//#endif
//
//
//
//            ofstream output("WideBoundary500Boundmatrix" + to_string(t) + ".csv");
//
//            output << "x, y, z, u" << "\n" << endl;
//
//
...
//            }
  ...
  */
    ...
    // proportions in different y regions:

  VectorXi regions = VectorXi::Zero(3);

    // when regions 120, 240, and the rest
    for (int j = 0; j < particles.size(); j++) {
      vdouble2 x = get<position>(particles[j]);
      if (x[1] >cell_radius + length_y && x[1] < length_y *2 - cell_radius) {
        regions(0) +=1;
      }
      if (x[1] >cell_radius + length_y/2 && x[1] < length_y *2 + length_y/2 - cell_radius) {
        regions(1) +=1;
      }
      if (x[1] >cell_radius  && x[1] < length_y *3 - cell_radius) {
        regions(2) +=1;
      }
    }


  regions(2) = regions(2) - regions(1); // only additional cells in that region

  regions(1) = regions(1) - regions(0);

  regions(0) = regions(0);
  regions(1) = regions(1);
  regions(2) = regions(2);;

  VectorXd fractions(3);

  fractions(0) = double(regions(0))/ double(particles.size());
  fractions(1) = double(regions(1))/ double(particles.size());
  fractions(2) = double(regions(2))/ double(particles.size());
...
int main(){
    ...
    int num_parts = 3; // for 1800 timesteps
...
#pragma omp parallel for
    for (int n = 0; n < sim_num; n++) {
        ...
        //ofstream output2(".//NEW REGIONS DATA//RegionsSeparateFullBoundary" + to_string(n) + ".csv");
...
//    ofstream output3(".//NEW REGIONS DATA//REGIONS600boundary.csv");
//
